...
export const registerUser = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (isEmailUsed) return next(new ErrorHandler(400, "Email is already used!"));
...
export const loginUser = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!email || !password) return next(new ErrorHandler(400, 'Please enter all the fields!'));
    const foundUser = await User.findOne({ email });
    if (!foundUser) return next(new ErrorHandler(400, 'User does not exist!'));
    const passwordMatched = await foundUser.comparePassword(password);
    if (!passwordMatched) return next(new ErrorHandler(400, 'Invalid credentials!'));
